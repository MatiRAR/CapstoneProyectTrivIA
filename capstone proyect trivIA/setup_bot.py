...
FILES = {
    ...
    "config.json": json.dumps({
        "features": {"text": True, "vision": True, "audio": True, "sentiment": True, "voice": True},
        "models": {
            "chat": "llama-3.3-70b-versatile",
            "vision": "meta-llama/llama-4-scout-17b-16e-instruct",
            "stt": "whisper-large-v3-turbo"
        },
        "runtime": {"json_mode": False}
    }, indent=2, ensure_ascii=False),
    "preguntas.json": json.dumps({
        "version": "1.0.0",
        "language": "es",
        "topics": [
            {"id": "politica", "preguntas": [
                "¿Qué opinás sobre la importancia de la separación de poderes?",
                "¿Qué indicadores usarías para evaluar la efectividad de una política pública?",
                "¿Qué mecanismos de participación ciudadana te parecen más útiles?"
            ]},
            {"id": "economia", "preguntas": [
                "¿Cómo impacta la inflación en la planificación financiera familiar?",
                "¿Qué políticas ayudan a fomentar las exportaciones de valor agregado?",
                "¿Cómo afecta la tasa de interés al crédito para pymes?"
            ]},
            {"id": "estudio", "preguntas": [
                "¿Qué método de estudio te funciona mejor (pomodoro, mapas mentales, Feynman)?",
                "¿Cómo definís objetivos semanales medibles para tus materias?",
                "¿Qué hábitos aplicás para retener conceptos a largo plazo?"
            ]},
            {"id": "tecnologia", "preguntas": [
                "¿Qué criterios usás para evaluar si una herramienta de IA vale la pena?",
                "¿Cómo definís un MVP tecnológico antes de escalar?",
                "¿Qué métricas usarías para medir el éxito de una automatización?"
            ]},
            {"id": "cultura", "preguntas": [
                "¿Qué obra te influyó y por qué?",
                "¿Cómo elegís contenido de calidad en medio de tanta oferta?"
            ]},
            {"id": "deportes", "preguntas": [
                "¿Qué deporte practicás y qué te aporta a nivel mental?",
                "¿Cómo medís progreso sin obsesionarte con resultados?"
            ]}
        ]
    }, indent=2, ensure_ascii=False),
}

# ======== CÓDIGO PRINCIPAL BOT (texto + visión + STT) ========
BOT_MAIN = dedent("""
...
# ======== ADAPTADORES ========
ADAPTER_SENTIMENT = dedent("""
def analyze_sentiment(text: str) -> dict:
    # TODO: reemplazar por tu implementación real
    return {"label": "POSITIVE", "score": 0.95, "text": text}
""").strip()

ADAPTER_VOICE = dedent("""
def tts_synthesize(text: str) -> dict:
    # TODO: reemplazar por tu TTS real
    return {"error": "TTS no implementado aún"}
""").strip()
...
with open("bot.py", "w", encoding="utf-8") as f:
    f.write(BOT_MAIN)
with open("modules/adapters_sentiment.py", "w", encoding="utf-8") as f:
    f.write(ADAPTER_SENTIMENT)
with open("modules/adapters_voice.py", "w", encoding="utf-8") as f:
    f.write(ADAPTER_VOICE)
with open("modules/__init__.py", "w", encoding="utf-8") as f:
    f.write("# init")

print(f"✅ Proyecto '{PROJECT_NAME}' creado correctamente.")
print("Siguiente paso:")
print("1️⃣  cd ai_telegram_groq")
print("2️⃣  pip install -r requirements.txt")
print("3️⃣  python bot.py")
print("⚙️  No olvides configurar tu .env con tus claves reales.")
